...
export class EditCookbook extends React.Component<EditCookbookProps> {
  ...
  render() {
    return <div className="form cookbook">
      ...
      {this.props.editCookbookController.isNew ? <button onClick={() => this.props.editCookbookController.addCookbook()} disabled={this.props.editCookbookController.name.replace(/\s/g, "").trimEnd().length < 3}>Add New Cookbook</button> :
        <button onClick={() => this.props.editCookbookController.updateCookbook()} disabled={this.props.editCookbookController.name.replace(/\s/g, "").trimEnd().length < 3}>Update Cookbook</button>}
    </div>
  }
}
